...
import CarPermitModel from "../model/CarPermitModel";
...
const getCarPermitById = async (req, res, id) => {
  try {
    const carPermit = await CarPermitModel.findById(id).populate("event card");
    if (!carPermit)
      return res.status(404).json({ error: "Car Permit not found" });
...
const getAllCarPermits = async (req, res) => {
  try {
    const carPermits = await CarPermitModel.find().populate("event card");
    if (carPermits.length === 0)
      return res.status(404).json({ message: "No Car Permits found" });
...
const createCarPermit = async (req, res) => {
  ...
  try {
    ...
    const newCarPermit = new CarPermitModel({
      event,
      card,
      originalImage,
      editedImage,
...
const updateCarPermit = async (req, res, id) => {
  try {
    const updatedCarPermit = await CarPermitModel.findByIdAndUpdate(
      id,
      req.body,
      { new: true }
    );
...
const deleteCarPermit = async (req, res, id) => {
  try {
    const deletedCarPermit = await CarPermitModel.findByIdAndDelete(id);
    if (!deletedCarPermit)
      return res.status(404).json({ error: "Car Permit not found" });
